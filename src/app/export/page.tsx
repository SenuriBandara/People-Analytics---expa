...
export default function Export() {
    ...
    // New state to toggle between analytics types
    const [isPeopleAnalytics, setIsPeopleAnalytics] = useState(false);
    ...
    function validateUrl() {
        ...
        if (
            isPeopleAnalytics &&
            !url.includes("expa.aiesec.org/analytics/people?")
        ) {
            setUrlError("Please provide a URL from the People Analytics page");
            setIsUrlValid(false);
            return false;
        }
        ...
        if (
            !isPeopleAnalytics &&
            !url.includes("expa.aiesec.org/analytics/performance?")
        ) {
            setUrlError("We only support performance analytics or people analytics for now.");
            setIsUrlValid(false);
            return false;
        }
    ...
    async function getData() {
        ...
        const urlParams = url.split("?")[1]; // Extract query parameters from the URL

        let formedUrl;
        if (isPeopleAnalytics) {
            formedUrl = `/api/export/people-analytics?${urlParams}`;
        } else {
            formedUrl = `/api/export/performance-analytics?${urlParams}&getPeople=${shouldGetPeople}&getApplications=${shouldGetApplications}`;
        }

        fetch(formedUrl)
            .then(async (data) => {
                if (!data.ok) throw new Error("Error fetching data. Please try again.");
    ...
    async function copyToClipBoard() {
        await navigator.clipboard.writeText(
            (await (blob as Blob).text()).replaceAll(",", "\t")
        );
        notifications.show({
            ...
            >
                    ...
                    <div className={`flex flex-col space-y-5 items-start w-full`}>
                        ...
                        <div className="flex space-x-4">
                            <label className="flex items-center space-x-2">
                                <input
                                    type="radio"
                                    checked={!isPeopleAnalytics}
                                    onChange={() => setIsPeopleAnalytics(false)}
                                />
                            ...
                            <label className="flex items-center space-x-2">
                                <input
                                    type="radio"
                                    checked={isPeopleAnalytics}
                                    onChange={() => setIsPeopleAnalytics(true)}
                                />
                        ...
                        <Input.Wrapper
                            label="EXPA URL"
                            description="Copy the URL from the EXPA analytics page"
                            error={urlError}
                            className={`w-full`}
                        >
                            <Input
                                placeholder="https://expa.aiesec.org/analytics/..."
                        ...
                        {modified && isUrlValid && !isPeopleAnalytics && (
                ...
                {dataAvailable && !modified && (
                    <div className={`flex flex-col space-y-5`}>
                        <div className={`text-xs text-gray opacity-50`}>
                            {url}
                        </div>
